...
@MODEL_REGISTRY.register()
class DAADMViT(nn.Module):
    """
    Model builder for DAAD-MViTv1 and DAAD-MViTv2.
    ...
    def __init__(self, cfg):
        ...
        self.x6_patch_embed = stem_helper.PatchEmbed(
            dim_in=in_chans,
            dim_out=embed_dim,
            kernel=cfg.MVIT.PATCH_KERNEL,
            stride=cfg.MVIT.PATCH_STRIDE,
        ...
        if self.cls_embed_on:
            self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
            self.x6_cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
            pos_embed_dim = num_patches + 1
        else:
            pos_embed_dim = num_patches
        ...
        if self.use_abs_pos:
            if self.sep_pos_embed:
                ...
                self.x6_pos_embed_spatial = nn.Parameter(
                    torch.zeros(1, self.patch_dims[1] * self.patch_dims[2], embed_dim)
                )
                self.pos_embed_temporal = nn.Parameter(
                    torch.zeros(1, self.patch_dims[0], embed_dim)
                )
                self.x6_pos_embed_temporal = nn.Parameter(
                    torch.zeros(1, self.patch_dims[0], embed_dim)
                )
                if self.cls_embed_on:
                    self.pos_embed_class = nn.Parameter(torch.zeros(1, 1, embed_dim))
                    self.x6_pos_embed_class = nn.Parameter(torch.zeros(1, 1, embed_dim))
                ...
                self.x6_pos_embed = nn.Parameter(
                    torch.zeros(
                        1,
                        pos_embed_dim,
                        embed_dim,
        ...
        if self.cls_embed_on:
            trunc_normal_(self.cls_token, std=0.02)
            trunc_normal_(self.x6_cls_token, std=0.02)
        self.apply(self._init_weights)
    ...
    def forward(self, x, bboxes=None, return_attn=False):
        ...
        if self.cls_embed_on:
            ...
            x6_cls_tokens = self.x6_cls_token.expand(
                B, -1, -1
            )
            if self.use_fixed_sincos_pos:
                cls_tokens = cls_tokens + self.pos_embed[:, :s, :]
                x6_cls_tokens = x6_cls_tokens + self.x6_pos_embed[:, :s, :]
                
            x = torch.cat((cls_tokens, x), dim=1)
            x6 = torch.cat((x6_cls_tokens, x6), dim=1)
        ...
        if self.use_abs_pos:
            if self.sep_pos_embed:
                ...
                x6_pos_embed = self.x6_pos_embed_spatial.repeat(
                    1, self.patch_dims[0], 1
                ) + torch.repeat_interleave(
                    self.x6_pos_embed_temporal,
                    self.patch_dims[1] * self.patch_dims[2],
                    dim=1,
                )
                ...
                if self.cls_embed_on:
                    pos_embed = torch.cat([self.pos_embed_class, pos_embed], 1)
                    x6_pos_embed = torch.cat([self.x6_pos_embed_class, x6_pos_embed], 1)

                x += self._get_pos_embed(pos_embed, bcthw)
                x6 += self._get_pos_embed(x6_pos_embed, bcthw)

            else:
                x += self._get_pos_embed(self.pos_embed, bcthw)
                x6 += self._get_pos_embed(self.x6_pos_embed, bcthw)
        ...
        if self.enable_rev:
            x = self._forward_reversible(x)
            x6 = self._forward_reversible(x6)
            ...
            if self.enable_detection:
                ...
                B, _, C = x.shape
                x = x.transpose(1, 2).reshape(B, C, thw[0], thw[1], thw[2])
                x6 = x6.transpose(1, 2).reshape(B, C, x6_thw[0], x6_thw[1], x6_thw[2])
                ...
                x = torch.concat([x, x6], dim=-1) #Late fusion is concat
                x = self.head(x)
