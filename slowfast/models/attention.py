...
def cal_rel_pos_spatial(attn, q, k, has_cls_embed, q_shape, k_shape, rel_pos_h, rel_pos_w, num_memory_tokens=0):
    """
...
def cal_rel_pos_temporal(attn, q, has_cls_embed, q_shape, k_shape, rel_pos_t, num_memory_tokens=0):
    """
...
class MultiScaleAttentionWithMemory(nn.Module):
    def __init__(
        self,
        dim,
        dim_out,
...
class MultiScaleBlockWithMemory(nn.Module):
    def __init__(
        self,
        dim,
        dim_out,
        ...
        padding_skip = [int(skip // 2) for skip in kernel_skip]
        att_dim = dim_out if dim_mul_in_att else dim
        self.attn = MultiScaleAttentionWithMemory(
            dim,
            att_dim,
            num_heads=num_heads,
            input_size=input_size,
