...
class Daadsequentialwithtime(torch.utils.data.Dataset):
    """
    ...
    def _construct_loader(self):
        ...
        with pathmgr.open(path_to_file, "r") as f:
            ...
            for clip_idx, row in enumerate(lines):
                ...
                assert len(fetch_info) >= 7, f"Invalid Data Annotation File."

                if len(fetch_info) == 9:
                    paths, label, start_time, end_time = fetch_info[:6], fetch_info[6], fetch_info[7], fetch_info[8]
                else:
                    paths, label, start_time, end_time = fetch_info[:6], fetch_info[6], None, None
